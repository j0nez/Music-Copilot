    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            pitches = [p + (octave - 4) * 12 for p in pitches]
            ...
            for i in range(total_notes):
                velocity = _bass_velocity(i, notes_per_bar, genre)
                notes_out.append({
                    "pitch": pitches[i],
                    "velocity": velocity,
...
def _bass_velocity(index: int, notes_per_bar: int, genre: str) -> int:
    beat = (index % notes_per_bar) + 1
    if beat == 1:
        return 105
    if beat == 3:
        return 95 if genre in ("house", "deep house") else 100
    if genre in ("dnb", "drum & bass"):
        return 78
    return 85
